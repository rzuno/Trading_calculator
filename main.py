...
# Buy models (gear_drop %, r). Adjust gear_drop if you prefer different depths.
BUY_MODELS = {
...
# Sell gears mapped by deployment ratio U
SELL_GEARS = [
...
def pick_sell_gear(u, emergency):
    if emergency:
        return {"name": "Emergency", "s": 1.0}
    for gear in SELL_GEARS:
        if gear["min_u"] <= u < gear["max_u"]:
            return gear
    return SELL_GEARS[-1]

def compute(avg_cost, current_units, max_units, buy_model_name, emergency):
    ...
    new_avg = (avg_cost * current_units + next_buy_price * buy_units) / new_units if new_units else avg_cost
    new_u = new_units / max_units if max_units else 0.0

    sell_gear = pick_sell_gear(u, emergency)
    s = sell_gear["s"] / 100
    sell_targets = [avg_cost * (1 + s), avg_cost * (1 + 2 * s), avg_cost * (1 + 3 * s)]
...
def on_show():
    ...
    data = compute(avg_cost, current_units, max_units, buy_model, emergency)
    ...
    result_lines = [
        ...
        f"Sell gear: {sg['name']} (+{s:.0f}%/+{2*s:.0f}%/+{3*s:.0f}%)",
...
ttk.Entry(main, textvariable=avg_cost_var, width=14).grid(row=0, column=1, sticky="w", padx=4, pady=4)

ttk.Label(main, text="Current Units").grid(row=1, column=0, sticky="e", padx=4, pady=4)
ttk.Entry(main, textvariable=current_units_var, width=14).grid(row=1, column=1, sticky="w", padx=4, pady=4)

ttk.Label(main, text="Max Units").grid(row=2, column=0, sticky="e", padx=4, pady=4)
ttk.Entry(main, textvariable=max_units_var, width=14).grid(row=2, column=1, sticky="w", padx=4, pady=4)
...
ttk.Button(main, text="Show Result", command=on_show).grid(row=5, column=0, columnspan=2, pady=10)
ttk.Label(main, textvariable=result_var, justify="left").grid(row=6, column=0, columnspan=2, sticky="w")

fig = Figure(figsize=(5.5, 3.5), dpi=100)
canvas = FigureCanvasTkAgg(fig, master=main)
canvas.get_tk_widget().grid(row=7, column=0, columnspan=2, pady=10, sticky="nsew")
